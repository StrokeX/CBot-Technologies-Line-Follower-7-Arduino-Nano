  ...
  if (Started) {
    if (UsePlan) {
      ...
      if (ComapareSensor(PLANS[CurIndex].SensorCondition)) {
        ...
        analogWrite(Lin1, 255);
        analogWrite(Lin2, 255);
        delay(PLANS[CurIndex].ActionDel);
...
void LineRobot::ReadSensors() {
  ...
  for (int i = 0; i < 7; i++) {
    if (SValues[i] < (SMax[i]*SensorSens) || SValues[i] < ManualSens) {
      SBit[i] = 1;
      active++;
    } else {
      SBit[i] = 0;
...
int LineRobot::ComapareSensor(int Con) {
  ...
  switch (Con) {
    ...
    case 19: return ((SBit[5] == 1 || SBit[6] == 1));
    case 20: return ((SBit[4] == 1 || SBit[5] == 1 || SBit[6] == 1));
    case 21: return ((SBit[3] == 1 || SBit[4] == 1 || SBit[5] == 1 || SBit[6] == 1));
    case 22: return ((SBit[0] == 1 || SBit[1] == 1 || SBit[2] == 1 || SBit[3] == 1));
    case 33: return ((SBit[0] == 1 || SBit[1] == 1 || SBit[2] == 1));
    case 24: return ((SBit[0] == 1 || SBit[1] == 1));
  }
  return false;
}
